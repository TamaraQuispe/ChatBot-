...
from app.views.page import render_software
...
<div class="p-2 bg-surface-container-low rounded-lg"><span class="material-symbols-outlined text-primary">devices</span></div>
<div class="text-right"><span class="text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">+3.2%</span></div>
</div>
...
<div class="glass-panel p-6 rounded-2xl shadow-sm hover:shadow-md transition-all">
<div class="flex justify-between items-start mb-4">
<div class="p-2 bg-surface-container-low rounded-lg"><span class="material-symbols-outlined text-primary">warning</span></div>
<div class="text-right"><span class="text-[11px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">Por vencer</span></div>
</div>
...
<div class="w-24 h-10"><svg class="w-full h-full" viewbox="0 0 100 40"><path class="sparkline-svg" d="M0,10 Q10,25 20,20 T40,35 T60,15 T80,25 T100,5" style="stroke: #f59e0b;"></path></svg></div>
</div>
</div>
</section>
<section class="mb-12">
...
_MODALS = (
    MODAL_DETALLE
    + MODAL_ELIMINAR.replace("$TITULO", "Eliminar Activo").replace("$MENSAJE", "Estas seguro de eliminar este activo?").replace("$ACTION", "/admin/salones/eliminar")
    + MODAL_ESTADO.replace("$ESTADO_ACTION", "/admin/salones/estado")
)

HTML_SOFTWARE = render_software(_CONTENT, extra_js=_JS, after_footer=_MODALS)
