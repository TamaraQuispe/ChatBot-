...
<title>Cambiar Contrasena - UTP</title>
...
<h1 class="text-white font-headline-lg text-headline-lg mb-4 font-bold">Cambio de Contrasena</h1>
<p class="text-white/80 text-body-md leading-relaxed">Tu administrador ha solicitado que actualices tu contrasena. Ingresa una nueva contrasena segura para continuar.</p>
</div>
</div>
...
<h2 class="font-headline-md text-headline-md text-on-surface font-bold">Nueva Contrasena</h2>
...
<p class="text-sm text-on-error-container font-medium" id="error-text">Error al cambiar la contrasena</p>
</div>
<form id="force-form" class="space-y-5" onsubmit="return cambiarContrasena(event)">
<div>
<label class="block text-sm font-bold text-secondary mb-1.5">Contrasena Temporal</label>
<div class="relative">
<input id="current-password" type="password" class="w-full px-4 py-3.5 border border-outline-variant rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary pr-12" placeholder="Ingresa la contrasena temporal" required>
...
<div>
<label class="block text-sm font-bold text-secondary mb-1.5">Nueva Contrasena</label>
<div class="relative">
...
<div>
<label class="block text-sm font-bold text-secondary mb-1.5">Confirmar Contrasena</label>
<input id="confirm-password" type="password" class="w-full px-4 py-3.5 border border-outline-variant rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary" placeholder="Repite la nueva contrasena" required>
</div>
...
if(np!==cp){document.getElementById('error-text').textContent='Las contrasenas no coinciden';document.getElementById('error-msg').classList.remove('hidden');return false}
...
.then(function(res){
if(res.data.error||res.status>=400){document.getElementById('error-text').textContent=res.data.error||'Error al cambiar la contrasena';document.getElementById('error-msg').classList.remove('hidden');btn.disabled=false;btn.querySelector('span').textContent='Cambiar Contrasena';document.getElementById('spinner').classList.add('hidden');return}
window.location.href='/chat'
})
.catch(function(){document.getElementById('error-text').textContent='Error de conexion';document.getElementById('error-msg').classList.remove('hidden');btn.disabled=false;btn.querySelector('span').textContent='Cambiar Contrasena';document.getElementById('spinner').classList.add('hidden')});
return false;
}
</script>
</body>
