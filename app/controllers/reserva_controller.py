...
class ReservaController:
    ...
    def procesar_reserva(self, id_usuario: int, id_espacio: int, fecha: str = "2026-05-28") -> bool:
        try:
            if not isinstance(id_usuario, int) or id_usuario <= 0:
                return False
            if not isinstance(id_espacio, int) or id_espacio <= 0:
                return False
            return self.gestor.crear_reserva(id_usuario, id_espacio, fecha)
        except Exception as e:
